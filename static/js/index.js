...
new Vue({
  ...
  methods: {
    ...
    // 保存消息到对应的频道
    saveMessage(tidings) {
      // 1若不存在当前线程房间的userid的，就添加新的.$set是vue添加属性的方法（target，key，value）
      if(!this.threads[tidings.threadId]) {
        this.$set(this.threads, tidings.threadId, []);
      }
      this.threads[tidings.threadId].push(tidings);

      // 1如果onLineUsers不存在这个userID就向onLineUsers数组中添加
